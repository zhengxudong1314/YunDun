...
public class MqttService extends Service implements MqttCallback {
    ...
    private static int MQTT_KEEP_ALIVE = 240000; // KeepAlive Interval in MS
    private static String MQTT_KEEP_ALIVE_TOPIC = "/users/keepalive"; // Topic format for KeepAlives
    private static byte[] MQTT_KEEP_ALIVE_MESSAGE = {0}; // Keep Alive message to send
    ...
    private static String[] userTopic;              //唯一的topic
    private static IMqttMsgListener listener;           //收到消息后的回调


    public static void init(String ip, int port, String[] topic, IMqttMsgListener l) {
        MQTT_BROKER = ip;
        MQTT_PORT = port;
        userTopic = topic;
        listener = l;
        Log.i(DEBUG_TAG, "init end, topic " + topic);
    }

    public static void configKeepAlive(int time, String topic, String content) {
        MQTT_KEEP_ALIVE = time;
        MQTT_KEEP_ALIVE_TOPIC = topic;
        if (content != null)
            MQTT_KEEP_ALIVE_MESSAGE = content.getBytes();
    }
    ...
    private synchronized void start() {
        ...
        Log.d(DEBUG_TAG, "start server! connect");
        connect();
...
//		    mClient = new MqttClient(url, mDeviceId);
    ...
    private synchronized void keepAlive() {
        if (isConnected()) {
            try {
                ...
                ex.printStackTrace();
                stop();
                if (listener != null) listener.onMqttException();
            } catch (MqttException ex) {
                ex.printStackTrace();
                stop();
                if (listener != null) listener.onMqttException();
    ...
    private synchronized void reconnectIfNecessary() {
        if (mStarted && mClient == null) {
            Log.d(DEBUG_TAG, "reconnectIfNecessary connect!");
            connect();
        }
    }
    ...
    private synchronized MqttDeliveryToken sendKeepAlive()
        ...
        if (mKeepAliveTopic == null) {
            mKeepAliveTopic = mClient.getTopic(MQTT_KEEP_ALIVE_TOPIC);
        }

        Log.i(DEBUG_TAG, "Sending Keepalive: " + MQTT_KEEP_ALIVE_TOPIC);
    ...
    public void messageArrived(MqttTopic topic, MqttMessage message)
            throws Exception {
        String messgString = new String(message.getPayload());
        Log.i(DEBUG_TAG, "  Topic:\t" + topic.getName() +
                "  msg:\t" + messgString);
        if (listener != null) {
            listener.onMqttReceive(messgString, topic.getName());
        }
    }
